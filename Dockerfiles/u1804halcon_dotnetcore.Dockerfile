...
# docker build /opt/halcon -f u1804halcon_dotnetcore.Dockerfile -t local/ubuntu:halcon_dotnetcore
...
# Creates only one layer, but copies unnecessary files. 
# COPY . $HALCONROOT

# This creates a layer for each COPY statement
COPY /bin $HALCONROOT/bin
COPY /lib/$HALCONARCH $HALCONROOT/lib/$HALCONARCH
COPY /license $HALCONROOT/license

# install necessary packages
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y git ca-certificates curl python

# NOTE: using dotnet, there should be no need for this
