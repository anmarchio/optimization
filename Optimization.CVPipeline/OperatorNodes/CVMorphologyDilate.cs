...
{
    [Serializable]
    public class CVMorphologyDilate : CVNode
    {
        public CVMorphologyDilate() : base()
        {

        }

        public CVMorphologyDilate(List<CVNode> children, float[] parameters) : base(children, parameters)
        {
        ...
        {
            output = new UMat();
            CvInvoke.MorphologyEx(input, output, Emgu.CV.CvEnum.MorphOp.Dilate, StructElement, Anchor, Iterations, Emgu.CV.CvEnum.BorderType.Default, new Emgu.CV.Structure.MCvScalar(0));
            return output;
        }
