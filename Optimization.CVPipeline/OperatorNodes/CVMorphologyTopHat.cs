...
{
    [Serializable]
    public class CVMorphologyTopHat : CVNode
    {
        public CVMorphologyTopHat() : base()
        {

        }

        public CVMorphologyTopHat(List<CVNode> children, float[] parameters) : base(children, parameters)
        {
        ...
        {
            output = new UMat();
            CvInvoke.MorphologyEx(input, output, Emgu.CV.CvEnum.MorphOp.Tophat, StructElement, Anchor, Iterations, Emgu.CV.CvEnum.BorderType.Default, new Emgu.CV.Structure.MCvScalar(0));
            return output;
        }
