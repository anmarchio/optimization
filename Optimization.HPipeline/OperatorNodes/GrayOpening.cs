...
{
    [Serializable]
    public class GrayOpening : HalconOperatorNode
    {
        public GrayOpening() : base() { }

        public GrayOpening(IList<HalconOperatorNode> children, float[] parameters) : base(children)
        {
        ...
        public override HObject Execute(HObject input)
        {
            ...
            {
                HOperatorSet.GrayOpening(input, str, out output);
                return output;
            }
        }
        ...
        public override List<string> HalconFunctionCall()
        {
            ...
            var grayopening = string.Format("gray_opening({0}, {1}, {2})", Children.First().OutputVariableName,
                structName, OutputVariableName);

            lines.Add(typeText);
            lines.Add(structString);
            lines.Add(grayopening);
