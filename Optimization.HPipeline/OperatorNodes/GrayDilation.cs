...
{
    [Serializable]
    public class GrayDilation : HalconOperatorNode
    {   
        public GrayDilation() : base() { }
             
        public GrayDilation(IList<HalconOperatorNode> children, float[] parameters) : base(children)
        {
        ...
        public override HObject Execute(HObject input)
        {
            ...
            {
                HOperatorSet.GrayDilation(input, str, out output);
                return output;
            }
        }
        ...
        public override List<string> HalconFunctionCall()
        {
            ...
            var graydilation = string.Format("gray_dilation({0}, {1}, {2})", Children.First().OutputVariableName,
                structName, OutputVariableName);

            lines.Add(typeText);
            lines.Add(structString);
            lines.Add(graydilation);
